...
- **`scripts/relink_telegram.sh`**: Repoints every Telegram bot at the current deployment — the master bot and each tenant bot whose token lives in the app vault. Reports `getWebhookInfo` after each call, because `setWebhook` returns `ok` for a URL Telegram can never reach and only `last_error_message` reveals that.
- **Push-to-deploy**: `cloudbuild.yaml` plus `scripts/setup_autodeploy.sh` build and deploy on push to `main`. The build deliberately sets neither `--set-env-vars` nor `--set-secrets`: `gcloud run deploy` preserves configuration it is not told to change, and rebuilding that list in CI — where the gitignored `.env` does not exist — would quietly deploy empty values. Configuration changes therefore remain a local `deploy_cloudrun.sh` run; the trigger ships code only.
- **`scripts/rotate_app_secrets.py`**: Rotates an application's credentials. The two are not the same kind of thing — `client_secret` is a password hash nothing else depends on, while `webhook_secret` doubles as the vault's encryption key (`utils/encryption.get_fernet` derives the Fernet key from it). Replacing the latter without re-encrypting orphans every value in `api_keys`, and since `decrypt_value` swallows failures and returns the ciphertext, nothing raises — the app simply receives an encrypted blob where its `MONGO_URI` should be. The script decrypts with the old secret and re-encrypts with the new one, refuses to run if any key fails to decrypt (re-encrypting those would double-wrap them and lose the plaintext permanently), and is a dry run unless `--apply`.
- **`scripts/set_tenant_db.py`**: Replaces a tenant's encrypted `db_connection` without going through the backoffice form, and refuses to write a string it cannot connect with. It also warns on a `db.<ref>.supabase.co` host: Supabase no longer provisions those on free projects — the name has neither an A nor an AAAA record, and resuming a paused project does not restore it — so any stored connection string built on one fails DNS permanently. The pooler host is the replacement.
